...
public static class NameGenerator
{
    ...
    public static string GenerateCrateName(Random random, IReadOnlyList<string> adjectives, IReadOnlyList<string> nouns)
    {
        ...
        var adjective = random.NextElement(adjectives);
        var noun = random.NextElement(nouns);
        var separator = random.NextBool(0.6) ? "-" : "_"; // Prefer hyphens
    ...
    public static string GenerateTerraformResourceName(Random random, string resourceType)
    {
        ...
        var numbers = random.NextBool(0.4) ? $"-{random.Next(1, 100):D2}" : "";

        var baseName = resourceType.Replace("aws_", "").Replace("azurerm_", "").Replace("_", "-");
        var suffix = random.NextElement(suffixes);
    ...
    {
        var device = random.NextElement(devices);
    ...
    public static string GenerateAwsArn(Random random, string service, string resourceType, string resourceId)
    {
        ...
        var accountId = random.Next(100000000, 999999999).ToString() + random.Next(100, 999);
    ...
    {
        return random.NextHexString(shortForm ? 7 : 40);
    }
    ...
    {
        var noun = random.NextElement(nouns).ToLowerInvariant();
        var suffix = random.NextHexString(5);
