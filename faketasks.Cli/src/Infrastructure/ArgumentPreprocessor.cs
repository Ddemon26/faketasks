...
public static class ArgumentPreprocessor {
    ...
    public static (string[] transformedArgs, string? moduleName) Preprocess(string[] args) {
        ...
        foreach (string flag in ModuleFlags.Keys) {
            if ( args.Contains( flag ) ) {
                string moduleName = ModuleFlags[flag];
                string[] transformed = TransformFlagToSubcommand( args, flag, moduleName );
                return (transformed, moduleName);
            }
        }

        // Check for combined short flags (-bt, -bs, etc.)
        string? combinedFlag = args.FirstOrDefault( a =>
                                                        a.StartsWith( "-" ) &&
                                                        a.Length > 2 &&
                                                        !a.StartsWith( "--" ) &&
        ...
        if ( combinedFlag != null ) {
            char moduleChar = combinedFlag[1];
            var moduleShortFlag = $"-{moduleChar}";

            if ( ModuleFlags.ContainsKey( moduleShortFlag ) ) {
                string moduleName = ModuleFlags[moduleShortFlag];
                string[] transformed = SplitCombinedFlags( args, combinedFlag, moduleName );
                return (transformed, moduleName);
            }
        }
    ...
    static string[] TransformFlagToSubcommand(string[] args, string flag, string moduleName) {
        List<string> result = new() { moduleName };
    ...
    static string[] SplitCombinedFlags(string[] args, string combinedFlag, string moduleName) {
        ...
        string remainingChars = combinedFlag.Substring( 2 );
    ...
    public static string GetModuleFlagsHelp() {
        IEnumerable<string> flags = ModuleFlags
            .GroupBy( kv => kv.Value )
