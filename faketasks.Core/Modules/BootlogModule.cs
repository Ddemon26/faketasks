...
{
    private readonly EmbeddedDataProvider _dataProvider;
    private LinuxData? _linuxData;
    ...
    /// </summary>
    private async Task<double> EmitKernelBanner(IModuleContext context, double bootTime, CancellationToken ct)
    {
        // Kernel version
    ...
    /// </summary>
    private async Task<double> EmitHardwareDetection(IModuleContext context, double bootTime, CancellationToken ct)
    {
        // CPU detection
        var cpuCount = context.Random.Next(2, 16);
        bootTime = await EmitMessage(context, bootTime, $"smpboot: CPU{cpuCount} CPUs detected", ConsoleColor.Green, 20, 60, ct);

        // Memory
        var memoryMb = context.Random.Next(8192, 65536);
        ...
        var deviceCount = context.Random.Next(5, 12);
        for (int i = 0; i < deviceCount; i++)
        {
            var device = context.Random.NextElement(_linuxData!.Devices);
            var deviceMsg = GenerateDeviceMessage(context, device);
        ...
        // Storage devices
        var storageDevice = context.Random.NextElement(new[] { "sda", "sdb", "nvme0n1" });
        var sizeGb = context.Random.Next(256, 2048);
    ...
    /// </summary>
    private async Task<double> EmitFilesystemMessages(IModuleContext context, double bootTime, CancellationToken ct)
    {
        ...
        // Swap
        var swapMb = context.Random.Next(2048, 8192);
    ...
    /// </summary>
    private async Task<double> EmitServiceStartup(IModuleContext context, double bootTime, CancellationToken ct)
    {
        // Select random services to start
        var serviceCount = context.Random.Next(8, 15);
        var servicesToStart = context.Random.NextSelection(_linuxData!.SystemdServices, Math.Min(serviceCount, _linuxData.SystemdServices.Count));

        foreach (var service in servicesToStart)
        {
            var status = context.Random.NextBool(0.95) ? "Started" : "Starting";
    ...
    /// </summary>
    private async Task<double> EmitBootComplete(IModuleContext context, double bootTime, CancellationToken ct)
    {
        bootTime = await EmitMessage(context, bootTime, "systemd[1]: Startup finished", ConsoleColor.Green, 80, 150, ct);

        var totalTime = TimestampHelper.FormatDuration(TimeSpan.FromSeconds(bootTime));
    ...
    /// </summary>
    private async Task<double> EmitMessage(
        ...
        CancellationToken ct)
    {
        ...
        var timestamp = TimestampHelper.FormatBootTime(bootTime);
        ...
        var delayMs = context.Random.Next(minDelayMs, maxDelayMs);
    ...
    /// </summary>
    private string GenerateDeviceMessage(IModuleContext context, string device)
    {
        ...
        else if (device.StartsWith("eth") || device.StartsWith("wlan"))
        {
            var mac = string.Join(":", Enumerable.Range(0, 6).Select(_ => context.Random.Next(0, 256).ToString("x2")));
            return $"{device}: link up, {context.Random.Next(100, 10000)} Mbps, MAC {mac}";
        }
        else if (device.StartsWith("tty"))
        {
            return $"{device}: initialized";
        }
        else
        {
            var message = context.Random.NextElement(_linuxData!.KernelMessages);
